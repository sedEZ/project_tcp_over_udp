...
void    receive_ack(int sockfd , int* my_port , struct sockaddr_in* client_addr ,seg* snd_s, seg* rcv_s, unsigned int* my_seq_num , unsigned int* my_ack_num);
...
void receive_ack(int sockfd , int* my_port , struct sockaddr_in* client_addr , seg* snd_s , seg* rcv_s ,unsigned int* my_seq_num , unsigned int* my_ack_num){
    ...
    if((n = recvfrom(sockfd,rcv_s,MSS,0,(struct sockaddr*)client_addr,&size))==-1){
        ERR_EXIT("Server:new_connection:send_files:recvfrom");
    }

    printf("\t\tReceive a packet (seq_num = %d , ack_num = %d)\n",rcv_s->header.seq_num,rcv_s->header.ack_num);
                    
    if(!is_ack(rcv_s)){
        ERR_EXIT("Server:new_connection:receive_ack:Invalid ACK");
    }
    else if(rcv_s->header.ack_num != (*my_seq_num)){
        printf("%d %d\n",rcv_s->header.ack_num , (*my_seq_num));
        ERR_EXIT("Server:new_connection:receive_ack:ack_num wrong");
    }
...
void set_seg(seg* s , uint16_t source_port , uint16_t des_port, uint32_t seq_num, uint32_t ack_num){
       ...
       s->header.code.head_len = sizeof(s->header);
}
